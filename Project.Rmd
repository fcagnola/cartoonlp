...
After gathering and cleaning the textual data we decided to perform a distant reading on it, aimed at finding recurrent patterns and schemes in poorly structured data. The methods we employed to perform our study are text mining and sentiment analysis and the experiments were carried out through the use of the following tools.
...
[^1]: cit. <https://mimno.github.io/Mallet/index>
...
This is the name chosen for an R language based package specifically targeted at natural language processing analyses. He package incorporates four different lexicons:
...
We imported `spacy`, `nltk` and `re` libraries performing NLP tasks such as PoS tagging and Name Entity Recognition. Their methods were used to clean up dialogues from character from non-lexical phenomena names and stop-words. For this purpose we first used spacy's `en_core_web_sm` dictionary to parse words and recognize characters' names labeled as `"PP"` and `"ORG"` by the parser, later nltk's pos tagger was used to recognize `"NN"` (ie., English nouns) with len() equal or greater than four characters and filter out all the other words.

After this step dialogues were fed back to MALLET, which produced new clusters to have meaningful word that made sense with one another to human readers.

Finally clusters were improved by removig words recognized as nouns, but containing symbols (in our particular case we had issues with apostrophes) in order to avoid redundancies -- inside and among clusters -- given by different forms of the same word.
...
Topic modelling is an unsupervised text mining method we decided to perform along with sentiment analysis. The technique of combining these two distant reading methods is at the state of the art observed to be prone to return interesting results.

First we imported th pre-processed files from directory `/nn_txts` into Mallet and removed English stop words -- if any -- detected with command `--remove-stopwords.`
...
In the exploratory phase we run MALLET multiple times over the corpus, parameters were modified from time to time considering the size and peculiarities of our corpus such as its limited dimension (i.e., 1.9 MB for 59 files), ans target-specificity of the language used in children movies. Thus, topics had to be appealing or at least related to kids' everyday life and likes (e.g., one topic could concern school, family, games emotions or different genres of tales).

We detected as useful input parameters for train-topics:

-   `–num-topics`: the actual number of topics created.

    Considering the abovementioned characteristics of the Disney corpus we set 5 as a starting alue and increased it up to 15, at this point we decided clusters where satisfying: each cluster was intelligible, homogeneous and words made sense with the movies they were assigned to

-   `–optimize-burn-in`: the number of iterations before hyper-parameter optimization begins. Default is twice the optimize interval.[^3]It was raised to to 60 (default was twice of interval, i.e., 40) since we noticed it helped with topics' homogenization.

    <br>

[^3]: cit. <https://mimno.github.io/Mallet/topics>
...
mallet_values <- read.csv("/Users/chiara/Desktop/final_file/mallet_values.csv")
...
1.  Topics spread allover the considered time range (e.g.,T5and T8)

2.  Topics concentrated in one or more specific timespans (e.g.,T10 and T13)
...
Here follows a list of the topics' words accompanied by tables showing the top N movies for each clusters based on the matrix.
...
 • **T15**, finally, proved to be an interesting cluster, as it looked like it could include many strong female leads. We decided to focus our attention on this train of thought and included in additional research all movies led by women to understand the difference in perception from 2000 to today. appears to be family-related language, and seems to be prominent in many movies. An interesting feature of this table is that there seems to be some kind of correlation between movies with a strong male vs female lead, where the former have fewer topic-related language while in the latter the weight is generally higher. This observation warrants additional analysis.
...
For this kind of analysis we extracted the dimensions of `df` we were interested in: *Release year*, *X* renamed as*Title*, and *Text* as it is the output of the first cleaning in Section 1.
...
The first step consistent in smoothing and re-scaling syuzhet vectors with `rescale()` and `loess()`functions respectively. At this point vectors still had different length vectors, that cannot be compared mathematically. Therefore we apply the same formula used in the tutorial, sampling vectors into 100 sampled points and plotting them on the graph.[^4]

[^4]: cf. <https://cran.r-project.org/web/packages/syuzhet/vignettes/syuzhet-vignette.html>
...
As we choose to plot dialogues in which T2 had weight \>= .40, we noticed that the lines of the two movies released in the mid-twentieth century have completely different shapes, suggesting there is no correlation between the fact they shared T2 and epoch and their sentiment trend. The graph becomes interesting when we also look at Tangled's curve (2010) that has the peculiarity of being a sort of historical average between the former movies.
...
Comparing the two movies at the poles of the timeline we consider here, we notice that the Peter Pan's curve is more excited and swinging with respect to the Treasure planet. The first Rescuers' movie, instead, looks like a pivot between the two of them suggesting that throughout years the plot of movies clustered in topic 3 might have been smoothed in terms of sentiment shifts.

Finally we have also plotted the sequel of the rescuers that was released 13 years apart from the original one. The comparison between these two movies is interesting because they start in two different way the first very positive, while the second is extremely negative, but they share the time position of local maxima and minima on the graph.
...
In the analysis of topic 9 we selected those movies released between the end of the ninetieths and the beginning of 2000 including Moana which has the highest weight value in this cluster. By means of this graph we found out that movies released after the year 2000 end with negative sentiment values, while Tarzan and Dinosar end with highly positive sentiment in their dialogues.

Additionally the graph shows the overall similarity of the curve of the first three movies that are close to each other in terms of time (1 \~ 4 years apart from each other) and how it evolved after 13 years with Moana. This last movie has indeed a more swinging curve, is the only one with values above 0 in the middle of the plot (others are positive only at the beginning and/or at the end of the story), and its curve decreases when others are increasing.
...
We suppose these values could be someway indicative of the changes in the approach of children (and also their parents) towards technology, .
...
For cluster 13 we analyzed three movies released between 1942 and 1950. Here we notice that the sentiment valence, at least at the beginning, is quite similar between Bambi and Cinderella, and never exceeds the value of 0 until the finale. On the other side Make Mine Music dialogues have a similar valence in the first half of the movie, but drastically changes at its half and increases in positivity reaching the value of 1 around point 70 in its narrative time sample.

This difference of MMM from Bambi and Cinderella might be explained with the fact that its structure (composed by 10 short-films), that does not undergo the same restrictions of the plot of a traditional story like the other two movies, but is representative of-- and influenced by the deliberate editing and artistic choices made by producers.
...
As we can see from the plotted graph we have an interesting situation in which the older film starts and ends with values below the 0 and reaches its maximum positivity in dialogues exactly in the middle of the narrative time. Menwhile the 1991's movie has a more complex curve in terms of emotional shifts and starts in a positive way.
...
The movies in cluster 15 have all similar curves and as time passes we notice an increasing of positive emotionaol valence expecially in the central part of the narrative time. The increase of positive values could be related to the increase of importance of the topic in the movies since it follows the same trend of weights:
